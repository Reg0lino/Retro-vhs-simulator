...
export const CrtCanvas: React.FC<CrtCanvasProps> = ({
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // --- NEW: Draw customizable OSD text & blend overlay BEFORE pixel & wave distortions ---
      ...
      // --- STAGE 8: CRT Viewfinder Bezel & Blinking REC camera overlays (high-res display!) ---
      if (settings.overlayType === "vhs_bezel") {
        drawVhsBezelOverlay(ctx, w, h, frameCount);
      } else if (settings.overlayType === "record_osd") {
        drawRecordOsdOverlay(ctx, w, h, frameCount);
      }
    ...
    const drawVhsBezelOverlay = (g: CanvasRenderingContext2D, w: number, h: number, frameCount: number) => {
      g.save();
      ...
      // Flashing battery meter level
      const batX = w - padX - 60;
      const batY = padY + 18;
      g.strokeStyle = "rgba(255, 255, 255, 0.85)";
      g.lineWidth = 2;
      g.strokeRect(batX, batY, 32, 16);
      g.fillRect(batX + 32, batY + 4, 3, 8);
      const blinkCell = Math.floor(frameCount / 25) % 3;
      g.fillStyle = "#10b981"; // Emerald green
      if (blinkCell >= 0) g.fillRect(batX + 3, batY + 3, 8, 10);
      if (blinkCell >= 1) g.fillRect(batX + 13, batY + 3, 8, 10);
      if (blinkCell >= 2) g.fillRect(batX + 23, batY + 3, 6, 10);

      g.restore();
    };

    const drawRecordOsdOverlay = (g: CanvasRenderingContext2D, w: number, h: number, frameCount: number) => {
      g.save();
      const dotBlink = Math.floor(frameCount / 25) % 2 === 0;
      if (dotBlink) {
        g.fillStyle = "#f43f5e"; // bright red REC bulb
        g.beginPath();
        g.arc(w * 0.08, h * 0.1, 7, 0, Math.PI * 2);
        g.fill();
      }
      g.fillStyle = "rgba(255, 255, 255, 0.9)";
      g.font = `bold 16px "Courier New", Courier, monospace`;
      g.fillText("● REC", w * 0.08 + 15, h * 0.1 + 5);

      // Programmatic recording duration stopwatch
      const totalSecs = Math.floor(frameCount / 30);
      ...
      const fStr = String(frameCount % 30).padStart(2, "0");
      g.fillText(`0:${hStr}:${mStr}:${sStr}.${fStr}`, w * 0.08, h * 0.1 + 32);
      g.restore();
    };
